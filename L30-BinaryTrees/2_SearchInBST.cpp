...
node* searchKey(node* root, int key) {
	...
	if (root->data == key) {
		...
		// key choti hai root se toh left subtree mei hi mill skti hai bas
		node* ans = searchKey(root->left, key);
		if (ans != NULL) { // mill gaya toh return krdo uska address
			return ans;
		}
		else { // nhi milla toh return krdo NULL ko
			return NULL;
		}
	}
	else { // Key badi hi hogi root se
		return searchKey(root->right, key);
		// node* ans = searchKey(root->right, key);
		// return ans;
	}
}
