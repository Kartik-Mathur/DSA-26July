...
void mirrorTree(node* root) {
	if (root == NULL) {
		// Tree hi nahi hai toh kuch nhi ho skta vapis jaao
		return;
	}

	// 1. Root ke left and right children ki values mei swap kr deta hoon
	swap(root->left, root->right);
	// 2. LST ko bolo ki krke de recursion
	mirrorTree(root->left);
	// 3. RST ko recursion ko bolo ki krke de
