...
class Pair { // pair inbuilt keyword hota toh Pair word ka pehle character capital rakhna
public:
	int height, diameter;
};
...
Pair fastDiameter(node* root) {
	...
	// LST se bolo pair dede height and max diameter ka
	...
	// Diameter
	int op1 = left.height + right.height; // ROOT ke through yeh hota hai diameter
	int op2 = left.diameter; // LST ka max diameter recursion ko bola hai dene ke liye
	int op3 = right.diameter; // RST ka max diameter recursion ko bola hai dene ke liye
	ans.diameter = max(op1, max(op2, op3));
	// Pair return krdo jismei height and diameter dono added hai
	return ans;
}
